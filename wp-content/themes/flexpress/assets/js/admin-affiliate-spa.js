...
(function() {
    ...
    const AdminAffiliateApp = {
        ...
        switchTab(tab) {
            ...
            document.querySelector(`[href="#${tab}"]`).classList.add('nav-tab-active');
        ...
        async loadAffiliates() {
            try {
                const response = await fetch(`${flexpress_admin.rest_url}flexpress/v1/admin/affiliates`, {
                    headers: {
        ...
        async loadTransactions() {
            try {
                const response = await fetch(`${flexpress_admin.rest_url}flexpress/v1/admin/transactions`, {
                    headers: {
                        'X-WP-Nonce': flexpress_admin.nonce
                    }
                });
                ...
                this.renderError('transactions-admin-app', 'Failed to load transactions');
            }
        },

        async loadPayouts() {
            try {
                const response = await fetch(`${flexpress_admin.rest_url}flexpress/v1/admin/payouts`, {
                    headers: {
                        'X-WP-Nonce': flexpress_admin.nonce
                    }
                });
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <span class="status-badge status-${affiliate.status || 'pending'}">
                                            ${(affiliate.status || 'pending').charAt(0).toUpperCase() + (affiliate.status || 'pending').slice(1)}
                                        </span>
                                    </td>
                                    <td>${(affiliate.commission_rate || 0)}%</td>
                                    <td>$${(affiliate.total_revenue || 0).toFixed(2)}</td>
                                    <td>
                                        <button class="button button-small" onclick="AdminAffiliateApp.editAffiliate(${affiliate.id})">
                                            Edit
                                        </button>
                                        <button class="button button-small" onclick="AdminAffiliateApp.toggleAffiliateStatus(${affiliate.id}, '${affiliate.status}')">
                        ...
                        <tbody>
                            ${this.state.transactions.map(transaction => `
                                <tr>
                                    ...
                                    <td>$${(transaction.revenue_amount || 0).toFixed(2)}</td>
                                    <td>$${(transaction.commission_amount || 0).toFixed(2)}</td>
                                    <td>
                        ...
                        <tbody>
                            ${this.state.payouts.map(payout => `
                                <tr>
                                    ...
                                    <td>$${(payout.payout_amount || 0).toFixed(2)}</td>
                                    <td>${payout.payout_method || ''}</td>
                                    <td>
                                        <span class="status-badge status-${payout.status || 'pending'}">
        ...
        async approvePayout(payoutId) {
            try {
                const response = await fetch(`${flexpress_admin.rest_url}flexpress/v1/admin/payouts/${payoutId}/approve`, {
                    method: 'POST',
                    headers: {
                        'X-WP-Nonce': flexpress_admin.nonce
                    }
        ...
        async denyPayout(payoutId) {
            try {
                const response = await fetch(`${flexpress_admin.rest_url}flexpress/v1/admin/payouts/${payoutId}/deny`, {
                    method: 'POST',
                    headers: {
                        'X-WP-Nonce': flexpress_admin.nonce
                    }
        ...
        async completePayout(payoutId) {
            try {
                const response = await fetch(`${flexpress_admin.rest_url}flexpress/v1/admin/payouts/${payoutId}/complete`, {
                    method: 'POST',
                    headers: {
                        'X-WP-Nonce': flexpress_admin.nonce
                    }
        ...
        async exportPayouts() {
            try {
                ...
                const url = `${flexpress_admin.rest_url}flexpress/v1/admin/payouts/export${method ? `?method=${method}` : ''}`;
                ...
                a.href = url_blob;
                a.download = `payouts-export-${data.method}-${new Date().toISOString().split('T')[0]}.csv`;
                document.body.appendChild(a);
                a.click();
                document.body.removeChild(a);
                window.URL.revokeObjectURL(url_blob);
                
                this.showNotice(`Payouts exported successfully (${data.method})`, 'success');
            } catch (error) {
        ...
        async toggleAffiliateStatus(affiliateId, currentStatus) {
            ...
            try {
                const response = await fetch(`${flexpress_admin.rest_url}flexpress/v1/admin/affiliates/${affiliateId}`, {
                    method: 'PATCH',
                    headers: {
                        'X-WP-Nonce': flexpress_admin.nonce,
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', () => AdminAffiliateApp.init());
    } else {
