...
(function ($) {
  ...
  function initEventHandlers() {
    ...
    $("#export-csv").on("click", function () {
      ...
      window.location.href =
        flexpressEarnings.ajaxurl + "?" + params.toString();
    });
  }
  ...
  function loadEarningsData(period, startDate, endDate) {
    ...
    $(
      ".flexpress-earnings-cards, .flexpress-earnings-charts, .flexpress-earnings-breakdown, .flexpress-earnings-transactions"
    ).css("opacity", "0.5");
  ...
  function updateDashboard(data) {
    ...
    $('[data-metric="gross_revenue"]').text(
      "$" + formatNumber(data.gross_revenue)
    );
    $('[data-metric="net_revenue"]').text("$" + formatNumber(data.net_revenue));
    $('[data-metric="total_transactions"]').text(
      formatNumber(data.total_transactions, 0)
    );
    $('[data-metric="affiliate_commissions"]').text(
      "$" + formatNumber(data.affiliate_commissions)
    );
    $('[data-metric="average_transaction"]').text(
      "$" + formatNumber(data.average_transaction)
    );
    $('[data-metric="refunds_chargebacks"]').text(
      "$" + formatNumber(data.refunds_total + data.chargebacks_total)
    );

    // Update breakdown table
    const breakdown = data.breakdown;
    $('[data-breakdown="subscriptions-count"]').text(
      breakdown.subscriptions.count
    );
    $('[data-breakdown="subscriptions-amount"]').text(
      "$" + formatNumber(breakdown.subscriptions.amount)
    );
    $('[data-breakdown="rebills-count"]').text(breakdown.rebills.count);
    $('[data-breakdown="rebills-amount"]').text(
      "$" + formatNumber(breakdown.rebills.amount)
    );
    $('[data-breakdown="unlocks-count"]').text(breakdown.unlocks.count);
    $('[data-breakdown="unlocks-amount"]').text(
      "$" + formatNumber(breakdown.unlocks.amount)
    );
    $('[data-breakdown="refunds-count"]').text(breakdown.refunds.count);
    $('[data-breakdown="refunds-amount"]').text(
      "-$" + formatNumber(breakdown.refunds.amount)
    );
    $('[data-breakdown="chargebacks-count"]').text(breakdown.chargebacks.count);
    $('[data-breakdown="chargebacks-amount"]').text(
      "-$" + formatNumber(breakdown.chargebacks.amount)
    );
    ...
    for (let i = 0; i < displayCount; i++) {
      ...
      tbody.append(`
                <tr>
                    ...
                    <td>${escapeHtml(
                      t.display_name || "N/A"
                    )}<br><small>${escapeHtml(t.user_email || "")}</small></td>
