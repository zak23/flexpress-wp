...
jQuery(document).ready(function($) {
    ...
    $('#add-affiliate-form').on('submit', function(e) {
        ...
        // Validate email
        if (!$affiliateEmail.val().trim() || !isValidEmail($affiliateEmail.val().trim())) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    showNotice(response.data.message || 'Affiliate added successfully!', 'success');
                    closeModal();
                    // Reload after a short delay to allow the notice to be seen
                    setTimeout(function() {
                        window.location.reload();
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showNotice(response.data.message || 'Error loading affiliate details', 'error');
                }
            },
            error: function(xhr, status, error) {
                console.error('AJAX Error:', status, error);
                    ...
                    <div class="modal-body">
                        <div class="affiliate-details-grid">
                            <div class="detail-section">
                                ...
                                <p><strong>Status:</strong> <span class="status status-${data.status}">${data.status.charAt(0).toUpperCase() + data.status.slice(1)}</span></p>
                                <p><strong>Affiliate Code:</strong> <code>${data.affiliate_code}</code></p>
                                <p><strong>Referral URL:</strong> <a href="${data.referral_url}" target="_blank">${data.referral_url}</a></p>
                            </div>
                            <div class="detail-section">
                                <h3>Commission Settings</h3>
                                <p><strong>Initial Commission:</strong> ${data.commission_initial}%</p>
                                <p><strong>Rebill Commission:</strong> ${data.commission_rebill}%</p>
                                <p><strong>Unlock Commission:</strong> ${data.commission_unlock}%</p>
                                <p><strong>Payout Threshold:</strong> $${data.payout_threshold}</p>
                            </div>
                            <div class="detail-section">
                                <h3>Payout Information</h3>
                                <p><strong>Method:</strong> ${data.payout_method.charAt(0).toUpperCase() + data.payout_method.slice(1).replace('_', ' ')}</p>
                                <p><strong>Details:</strong> ${data.payout_details}</p>
                            </div>
                            <div class="detail-section">
                                <h3>Performance Statistics</h3>
                                <p><strong>Total Clicks:</strong> ${data.total_clicks}</p>
                                <p><strong>Total Signups:</strong> ${data.total_signups}</p>
                                <p><strong>Total Revenue:</strong> $${data.total_revenue}</p>
                                <p><strong>Total Commission:</strong> $${data.total_commission}</p>
                                <p><strong>Pending Commission:</strong> $${data.pending_commission}</p>
                                <p><strong>Approved Commission:</strong> $${data.approved_commission}</p>
                                <p><strong>Paid Commission:</strong> $${data.paid_commission}</p>
                            </div>
                            ${data.notes ? `<div class="detail-section"><h3>Notes</h3><p>${data.notes}</p></div>` : ''}
                        </div>
                        <div class="modal-actions">
                            <button type="button" class="button button-primary edit-affiliate-from-details" data-id="${data.id}">
                                Edit Affiliate
                            </button>
    ...
    function showEditAffiliateModal(data) {
                    ...
                    <div class="modal-body">
                        <form id="edit-affiliate-form">
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" id="edit-affiliate-name" name="display_name" value="${data.display_name}" required>
                            </div>
                            <div class="form-field">
                                <label for="edit-affiliate-email">Email Address *</label>
                                <input type="email" id="edit-affiliate-email" name="email" value="${data.email}" required>
                            </div>
                            <div class="form-field">
                                <label for="edit-affiliate-website">Website/Social Media</label>
                                <input type="url" id="edit-affiliate-website" name="website" value="${data.website || ''}" placeholder="https://example.com">
                            </div>
                            <div class="form-field">
                            ...
                            <div class="form-field">
                                ...
                                <input type="text" id="edit-payout-details" name="payout_details" value="${data.payout_details}" required placeholder="PayPal email, bank account, etc.">
                            </div>
                            <div class="form-field">
                                <label for="edit-commission-initial">Initial Commission Rate (%)</label>
                                <input type="number" id="edit-commission-initial" name="commission_initial" min="0" max="100" step="0.1" value="${data.commission_initial}">
                            </div>
                            <div class="form-field">
                                <label for="edit-commission-rebill">Rebill Commission Rate (%)</label>
                                <input type="number" id="edit-commission-rebill" name="commission_rebill" min="0" max="100" step="0.1" value="${data.commission_rebill}">
                            </div>
                            <div class="form-field">
                                <label for="edit-commission-unlock">Unlock Commission Rate (%)</label>
                                <input type="number" id="edit-commission-unlock" name="commission_unlock" min="0" max="100" step="0.1" value="${data.commission_unlock}">
                            </div>
                            <div class="form-field">
                                <label for="edit-payout-threshold">Payout Threshold ($)</label>
                                <input type="number" id="edit-payout-threshold" name="payout_threshold" min="0" step="0.01" value="${data.payout_threshold}">
                            </div>
                            <div class="form-field">
                            ...
                            <div class="form-field">
                                ...
                                <textarea id="edit-affiliate-notes" name="notes" rows="3" placeholder="Additional notes about this affiliate...">${data.notes || ''}</textarea>
                            </div>
        ...
        $modal.find('#edit-affiliate-form').on('submit', function(e) {
            ...
            const formData = {
                ...
                payout_method: $payoutMethod.val(),
                payout_details: $payoutDetails.val().trim(),
                commission_initial: parseFloat($('#edit-commission-initial').val()) || 25,
                commission_rebill: parseFloat($('#edit-commission-rebill').val()) || 10,
                commission_unlock: parseFloat($('#edit-commission-unlock').val()) || 15,
                payout_threshold: parseFloat($('#edit-payout-threshold').val()) || 100,
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        showNotice(response.data.message || 'Affiliate updated successfully!', 'success');
                        closeEditAffiliateModal();
                        // Reload after a short delay to allow the notice to be seen
                        setTimeout(function() {
                            window.location.reload();
                        ...
                        showNotice(response.data.message || 'Error updating affiliate', 'error');
                    }
                },
                error: function(xhr, status, error) {
                    console.error('AJAX Error:', status, error);
    ...
    function showStatsModal(data) {
        ...
        if (data.timeline && data.timeline.length && typeof Chart !== 'undefined') {
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="edit-promo-form">
                            <div class="form-field">
                                <label for="edit-affiliate-name">Affiliate Name</label>
                                <input type="text" id="edit-affiliate-name" name="affiliate_name" value="${data.affiliate_name || ''}" required>
                            </div>
                            <div class="form-field">
                                <label for="edit-target-plans">Target Plans</label>
                                <select id="edit-target-plans" name="target_plans[]" multiple required>
                                    ${data.target_plans ? data.target_plans.map(plan => `<option value="${plan}" selected>${plan}</option>`).join('') : ''}
                                </select>
                            </div>
                            <div class="form-field">
                                <label for="edit-commission-rate">Commission Rate (%)</label>
                                <input type="number" id="edit-commission-rate" name="commission_rate" min="0" max="100" step="0.1" value="${data.commission_rate || 10}" required>
                            </div>
                            <div class="form-actions">
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        showFormError($affiliateName, response.data.message || flexpressAffiliate.i18n.error);
                    }
                },
                error: function(xhr, status, error) {
                    console.error('AJAX Error:', status, error);
            ...
            success: function(response) {
                if (response.success && response.data) {
                    ...
                    response.data.forEach(plan => {
                        $select.append(`<option value="${plan.name}">${plan.name}</option>`);
                    });
                }
            }
        });
    ...
    $('#create-payout-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    showNotice(response.data.message || 'Payout created successfully!', 'success');
                    closeModal();
                    loadPayouts();
                } else {
                    showNotice(response.data.message || 'Error creating payout', 'error');
                }
            },
            error: function(xhr, status, error) {
                console.error('AJAX Error:', status, error);
    ...
    $('#update-payout-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    showNotice(response.data.message || 'Payout status updated successfully!', 'success');
                    closeModal();
                    loadPayouts();
                } else {
                    showNotice(response.data.message || 'Error updating payout status', 'error');
                }
            },
            error: function(xhr, status, error) {
                console.error('AJAX Error:', status, error);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    showNotice(response.data.message || 'Error loading payout details', 'error');
                }
            },
            error: function(xhr, status, error) {
                console.error('AJAX Error:', status, error);
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    $tbody.html('<tr><td colspan="7">Error loading payouts: ' + (response.data.message || 'Unknown error') + '</td></tr>');
                }
            },
            error: function(xhr, status, error) {
                console.error('AJAX Error:', status, error);
                $tbody.html('<tr><td colspan="7">Error loading payouts: ' + error + '</td></tr>');
            }
        });
    }
    ...
    function renderPayoutsTable(payouts) {
        ...
        payouts.forEach(function(payout) {
            ...
            html += '<tr>';
            html += '<td><strong>' + escapeHtml(payout.affiliate_name) + '</strong><br><small>' + escapeHtml(payout.affiliate_email) + '</small></td>';
            html += '<td>' + escapeHtml(period) + '</td>';
            html += '<td>$' + parseFloat(payout.payout_amount).toFixed(2) + '</td>';
            html += '<td>' + escapeHtml(payout.payout_method.charAt(0).toUpperCase() + payout.payout_method.slice(1).replace('_', ' ')) + '</td>';
            html += '<td><span class="status ' + statusClass + '">' + statusLabel + '</span></td>';
            html += '<td>' + createdDate + '</td>';
            html += '<td>';
            html += '<button type="button" class="button button-small view-payout" data-id="' + payout.id + '">View</button> ';
            html += '<button type="button" class="button button-small update-payout-status" data-id="' + payout.id + '" data-status="' + payout.status + '">Update</button>';
            html += '</td>';
            html += '</tr>';
        });
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    response.data.forEach(function(affiliate) {
                        $select.append('<option value="' + affiliate.id + '">' + escapeHtml(affiliate.display_name) + ' ($' + parseFloat(affiliate.pending_amount).toFixed(2) + ' pending)</option>');
                    });
                }
            },
            error: function(xhr, status, error) {
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <div class="affiliate-details-grid">
                            <div class="detail-section">
                                <h3>Payout Information</h3>
                                <p><strong>Affiliate:</strong> ${data.affiliate_name}</p>
                                <p><strong>Email:</strong> ${data.affiliate_email}</p>
                                <p><strong>Period:</strong> ${data.period_start} to ${data.period_end}</p>
                                <p><strong>Amount:</strong> $${parseFloat(data.payout_amount).toFixed(2)}</p>
                                <p><strong>Status:</strong> <span class="status status-${data.status}">${data.status.charAt(0).toUpperCase() + data.status.slice(1)}</span></p>
                            </div>
                            <div class="detail-section">
                                ...
                                ${data.processed_at ? '<p><strong>Processed:</strong> ' + new Date(data.processed_at).toLocaleString() + '</p>' : ''}
                            </div>
                        </div>
                        ${data.transactions.length > 0 ? `
                        <div class="detail-section">
                            <h3>Related Transactions (${data.transactions.length})</h3>
                                ...
                                <tbody>
                                    ${data.transactions.map(t => `
                                        <tr>
                                            ...
                                            <td>$${parseFloat(t.revenue_amount).toFixed(2)}</td>
                                            <td>$${parseFloat(t.commission_amount).toFixed(2)}</td>
                        ...
                        ${data.notes ? `<div class="detail-section"><h3>Notes</h3><p>${data.notes}</p></div>` : ''}
                        <div class="modal-actions">
                            <button type="button" class="button button-primary update-payout-status" data-id="${data.id}" data-status="${data.status}">
                                Update Status
                            </button>
                            <button type="button" class="button modal-close">Close</button>
                        </div>
    ...
    $(document).on('click', '.approve-affiliate, .reject-affiliate, .suspend-affiliate, .reactivate-affiliate', function() {
        ...
        const action = $button.hasClass('approve-affiliate') ? 'active' :
                      $button.hasClass('reject-affiliate') ? 'rejected' :
                      $button.hasClass('suspend-affiliate') ? 'suspended' :
                      $button.hasClass('reactivate-affiliate') ? 'active' : 'pending';
        
        const actionText = $button.hasClass('approve-affiliate') ? 'approve' :
                         $button.hasClass('reject-affiliate') ? 'reject' :
                         $button.hasClass('suspend-affiliate') ? 'suspend' :
                         $button.hasClass('reactivate-affiliate') ? 'reactivate' : 'update';
        ...
        if (action === 'rejected' || action === 'suspended') {
            notes = prompt(`Please provide a reason for ${actionText}ing this affiliate:`);
            if (notes === null) {
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    showNotice(response.data.message || `Affiliate ${actionText}d successfully!`, 'success');
                    // Reload the page to update the status
                    setTimeout(function() {
                        window.location.reload();
                    }, 1000);
                } else {
                    showNotice(response.data.message || `Error ${actionText}ing affiliate`, 'error');
                }
            },
            error: function(xhr, status, error) {
                console.error('AJAX Error:', status, error);
            ...
            complete: function() {
                $button.prop('disabled', false).html($button.text().replace('ing...', ''));
            }
        });
    });
