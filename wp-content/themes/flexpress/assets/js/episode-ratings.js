...
jQuery(document).ready(function ($) {
  ...
  function submitRating(episodeId, rating, comment) {
    const $form = $(
      '.episode-rating-form[data-episode-id="' + episodeId + '"]'
    );
    const $stars = $form.find(".rating-stars-interactive .star");
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          // Reset form
          $form
            .find(".rating-stars-interactive .star")
            .removeClass("filled half-filled");
  ...
  function updateRatingStats(episodeId, stats) {
    ...
    $statsContainer
      ...
      .text(
        `(${stats.total_ratings} ${
          stats.total_ratings === 1 ? "rating" : "ratings"
        })`
      );
    ...
    Object.keys(stats.rating_distribution).forEach(function (rating) {
      ...
      const $bar = $statsContainer.find(
        `.rating-bar:nth-child(${6 - rating}) .progress-bar`
      );
      $bar.css("width", percentage + "%");
      $statsContainer
        .find(`.rating-bar:nth-child(${6 - rating}) .rating-count`)
        .text(count);
    });
  }
