...
class FlowguardValidation {
  constructor(paymentForm, options = {}) {
    ...
    this.helpMessages = {
      ...
      remember:
        "Check this box to securely save your card for faster future payments",
    };
  ...
  validateCardholder(value, isBlur = false) {
    if (!value || value.trim() === "") {
      return {
        isValid: false,
        message: this.errorMessages.CARDHOLDER_REQUIRED,
      };
    }
  ...
  showValidationSummary() {
    let summaryContainer = document.getElementById("validation-summary");
    if (!summaryContainer) {
      ...
      const validationContainer = document.getElementById(
        "flowguard-validation-container",
      );
      if (validationContainer) {
        validationContainer.insertBefore(
          container,
          validationContainer.firstChild,
        );
      }
    ...
    if (errorCount > 0) {
      summaryContainer.innerHTML = `
                ...
                <ul>
                    ${Object.values(this.validationState.errors)
  ...
  shouldAutoRetry(errorCode) {
    ...
    return (
      autoRetryErrors.includes(errorCode) &&
