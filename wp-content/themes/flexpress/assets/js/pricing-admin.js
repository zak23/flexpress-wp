jQuery(document).ready(function ($) {
  ...
  function validateField($field) {
    ...
    if (fieldName === "trial_duration" && value) {
      ...
      const trialDurationUnit = $("#trial-duration-unit").val();
      ...
      // Verotel requirement: minimum 3 days
      if (trialDurationUnit === "days" && trialDuration < 3) {
        showError(
          $field,
          "Trial period must be at least 3 days (Verotel requirement)"
        );
        return false;
      }
    }
  ...
  $(document).on(
    "click",
    ".pricing-modal-close, #cancel-plan-edit",
    function () {
  ...
  // Handle trial duration unit change to adjust min attribute
  function updateTrialDurationMin() {
    const trialDurationUnit = $("#trial-duration-unit").val();
    const $trialDurationField = $("#trial-duration");

    // Verotel requirement: minimum 3 days
    if (trialDurationUnit === "days") {
      $trialDurationField.attr("min", "3");
    } else {
      $trialDurationField.attr("min", "1");
    }

    // Revalidate field if it has a value
    if ($trialDurationField.val()) {
      validateField($trialDurationField);
    }
  }

  // Bind trial duration unit change event
  $("#trial-duration-unit").change(updateTrialDurationMin);

  // Also validate when trial duration changes
  $("#trial-duration").on("input change", function () {
    validateField($(this));
  });
  ...
  $form.submit(function (e) {
    ...
    if (planType === "one_time" || planType === "lifetime") {
      ...
      if ($trialEnabled.is(":checked")) {
        const planTypeText =
          planType === "lifetime" ? "lifetime access" : "one-time payments";
        showError(
          $trialEnabled,
          "Trial periods are not allowed for " + planTypeText
        );
        isValid = false;
      }
    } else {
  ...
  function openModal(mode, planId = null) {
    ...
    if (mode === "add") {
      ...
      console.log(
        "New plan defaults set - active:",
        $("#plan-active").prop("checked")
      );
  ...
  function savePlan() {
    ...
    if (typeof flexpressPricing === "undefined") {
      ...
      showNotice(
        "error",
        "Configuration error: AJAX settings not found. Please refresh the page."
      );
      return;
    }
    ...
    const finalData =
      formData +
      additionalData +
      "&action=save_pricing_plan&nonce=" +
      flexpressPricing.nonce;
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          ...
          $(`.pricing-plan-card[data-plan-id="${planId}"]`).fadeOut(
            function () {
              $(this).remove();
      ...
      success: function (response) {
        if (response.success) {
          ...
          if ($card.hasClass("inactive")) {
            ...
            $card
              .find(".plan-title")
              .append('<span class="plan-badge inactive">Inactive</span>');
          }
        } else {
          showNotice("error", response.data || flexpressPricing.strings.error);
        }
  ...
  function validateField($field) {
    ...
    if (!isValid) {
      $field.addClass("error");
      $field.after(
        `<span class="error-message" style="color: #d63638; font-size: 12px; display: block; margin-top: 2px;">${errorMessage}</span>`
      );
    }
    ...
    $.ajax({
      ...
      success: function (response) {
        if (response.success) {
          showNotice(
            "success",
            "Flowguard connection successful! " + response.data
          );
        } else {
      ...
      error: function () {
        showNotice(
          "error",
          "Failed to test Flowguard connection. Please try again."
        );
      },
      complete: function () {
  ...
  function validatePricingPlans() {
    ...
    $.ajax({
      ...
      error: function () {
        showNotice(
          "error",
          "Failed to validate pricing plans. Please try again."
        );
      },
      complete: function () {
