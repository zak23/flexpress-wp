...
{
    // Only load on edit screens that actually use the repeater mod, and avoid model edits
    if ($hook === 'post.php' || $hook === 'post-new.php') {
        // Prefer get_current_screen when available to detect post type reliably
        if (function_exists('get_current_screen')) {
            $screen = get_current_screen();
            if ($screen && isset($screen->post_type) && $screen->post_type === 'model') {
                return; // Do not enqueue on model edit screens to avoid ACF collisions
            }
        }
