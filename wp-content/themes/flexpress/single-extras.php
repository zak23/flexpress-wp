...
while (have_posts()):
    ...
    // Note: We don't redirect here - let the preview system handle access control
    // Non-logged-in users can see preview content (first 5 images or preview video)
                    ...
                    <div class="extras-details mb-4">
                        ...
                        <?php elseif ($content_format === 'video'): ?>
                            ...
                            <div class="video-player mb-4">
                                ...
                                if ($has_access && $video_id):
                                    // Generate token and expiration for full video
                                ...
                                <?php elseif (!$has_access && $preview_video): ?>
                                    <!-- Preview video for users without access -->
                                    <?php
                                    // Generate token and expiration for preview video
                                    $expires = time() + 3600; // 1 hour expiry
                                    $token = '';
